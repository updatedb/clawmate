...
1. **远程文件管理** — 以系统根目录及按用户授权的子目录为入口，覆盖上传、搜索、预览、移动、分享和下载等完整文件生命周期。
...
| 图片 | ‹ › 导航切换 + 缩略图大纲（WebP 派生缩略图、原图详情可验证缓存） |
| 音视频 | 内嵌播放器 + 字幕同步 |
...
```json
{
  "system_root_dir": "/data/mounted-workspace"
}
```
...
> ⚠️ **`auth.local_hosts` 授予的是完整管理员权限，不只是文件访问。** 命中本机绕过的客户端会被绑定为 `is_admin=true` 的 `local-admin` 主体，因此账户列表、创建/删除用户、修改授权等设置类接口对它同样开放。仅当这些主机确实是你独占的可信机器时才填入（支持主机名或 IP，主机名会经 DNS 解析后比对）。留空 `[]` 即只保留回环地址（`127.0.0.1` / `::1`）。
>
> 注意 `local_hosts` 与 `_is_local_network_host()` 是**两件不同的事**：后者只影响 WS 地址/公网回退的计算（按 `.lan` / 私有网段判定），不参与认证绕过决策。
