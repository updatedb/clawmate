...
- 新增 `clawmate project <projectname>` 命令
- 搜索项目目录 → 确定 agent_id
- v1: sessions_spawn subagent（不可用，用户看不到）
- v2: sessions_send → agent:agent_id:main（用户可在 /sessions 切换到该会话）
- SKILL.md 更新，章节号 6/7/8 重新编号
- _meta.json v2.3.0 → v2.4.1，clawmate_project + agent 映射表

## v1.33 统一 task/run 入口 + 安全加固 + Mobile 同步 ✅ (commit: 5c18a91)
...
## v1.37 文档工程化 + 依赖修复 + 协作流程文档 ✅ (commit: 04db23a)
- 新增 ClawMate + OpenClaw 协作流程图（install→init→project→review→fix）
- 更新架构图与 Slash Commands 表
- 替换截图网格为单张协作闭环截图
- 恢复 mapping table 并提交所有积压变更
- fix: add python-multipart to requirements.txt（上传 multipart 缺失依赖）

## v1.38 桌面版 Ctrl+V 剪切板图片直接上传 ✅ (commit: 4a98239)
- 桌面版 Ctrl+V 监测 clipboard 图片，直接上传到当前目录
- 上传后自动跳转到文件预览
- 在底部工具栏添加状态提示

## v1.39 CDN 依赖去网络化 + Mobile markdown-it guard ✅ (commit: d77d2db)
- chore: markdown-it 插件从 CDN 下载到本地 vendor/ 目录（container/emoji/footnote/task-lists）
- fix(mobile): markdown-it 插件初始化 fallback guard（undefined 保护）
- 移除所有运行时 CDN 依赖，vendor 目录自包含

## v1.40 响应式策略统一 — Agent 面板渐进隐藏 ✅ (commit: v1.40)
...
- 列表视图日期缩短 M/D HH:mm、minmax 自适应列宽

## v1.41 压缩包预览 + 文件移动 + Feedback 隐藏 ✅ (commit: 295316e)
- 压缩包预览：zip/tar/tar.gz/tar.bz2/tar.xz/rar/7z 树形展开
- 文件移动：POST /api/clawmate/move 同 root 内移动文件/目录
- 切换 root 时侧边栏和面包屑正确刷新（缓存 key 改为 rootId:dir）
- feedback.json → .feedback.json，默认隐藏

## v1.42 图标系统重构 + Feedback 迁移 + API 扩展 ✅ (HEAD: 061a212)
- 图标: 彩色字母标签(M/Py/$/{}/T/JS)、_tag() 参数化、画廊32px/列表22px
- Feedback: 迁移至 .clawmate/feedback.json，marker 严格校验（无 .clawmate/ 报错）
- 新增 API: /api/clawmate/link(搜索+预览链接)、/api/clawmate/mkdir(创建目录)
- Agent: PTY 60fps 分组刷新、session key 同步、用户输入 echo 优先
- Auth: local_hosts LAN 免登录配置、侧边栏 . 开头 dotfile 过滤
- README: 全面审查更新，核心能力重写为 6 大产品 Feature（项目管理/Agent面板/Slash Commands）
- Skill: v2.6.0 → v2.7.0 发布 ClawHub（list [root_id] 参数修正）
- 配置: config.json 示例补全 agent/local_hosts/feedback.tags

## v1.43 Mermaid 高度调整 + xterm 输入延迟修复 ✅ (commit: 5ef66dd)
- Mermaid 图表底部拖拽手柄，支持鼠标/触摸拖拽调整显示区域高度
- DOM 结构重构：.mermaid-inner 承载滚动内容，handle 和 zoom controls 固定在外层
- xterm 输入从 30ms debounce 改为立即 flush，消除 PTY echo 延迟

## v1.44 Codex 后端 + 画廊菜单重构 + 文件移动 + translate 动画 + xterm 流控 ✅ (commit: dd8a4c5)
- Agent: 新增 Codex 作为第三 Agent 后端，session key 重构为 {backend}:{root}[:{project}] 隔离
- Agent: get_agent_session 遍历所有 PTY 后端；max_sessions/env 配置；CLAWMATE_AGENT_BACKEND env
- Agent: onFlowControlPause/Resume 流控（写入缓冲超限丢弃帧）；term.blur() 暂停光标定时器
- UI: 卡片操作从 card-actions 迁移到 ⋯ 菜单按钮（Dropdown 浮层）；复选框移入 thumb 自定义样式
- UI: 列表视图行内复制按钮；面包屑刷新按钮
- UI: 目录选择器 Modal，画廊 ⋯ 菜单集成 move 操作
- Feat: POST /api/clawmate/move 端点 + service.move_file()（安全校验：防自身/子目录/同名冲突）
- CSS: 面板隐藏改用 translate 替代 width/opacity（GPU 合成层）；share-view TOC slide 动效
- Deps: +websockets>=12.0, -aiofiles；清理无用 vendor（radar.mjs）
- Docs: README 全面审计更新（三后端/架构图/config 示例对齐/Skill 安装命令统一）；CHANGELOG v1.44
- Chore: .gitignore 增加 .claude/

## v1.45 Agent 后端切换 + Mermaid 弹窗 + Mobile 完善 ✅ (commit: 2026-06-27)
- Agent: badge 循环切换后端 claude→codex→openclaw，前后端 WS 联动
- Agent: header 精简（移除 root 标签）、Ctrl+L 清屏改为 clear 按钮
- Agent: 面板宽度 700→800px（拖拽上限 900px）
- Agent: ansi-up DOM 渲染器实验→回退纯 xterm；CSS scale 方案→回退
- xterm: 多屏适配（实测字符宽度+COLUMNS/LINES）、面板宽度统一
- Mermaid: 弹窗查看按钮 + 弹窗内缩放/拖拽/居中 + 滚轮直接缩放
- Mobile: z-index 修复、目录点击关闭面板、overflow:hidden 锁定视口、bfcache 恢复
- PWA: manifest + Service Worker + Agent 文件上下文 + 无限滚动 + 主题防闪烁
- UI: 返回按钮 SVG 统一、主题图标 sun→sun-moon、图片导航固定视口
- Docs: README 截图全面更新、SKILL.md v2.7.2


## v1.47 Preview &line=N + 搜索清除 + 内容匹配面板 + 源码高亮 ✅ (commit: pending)
- Preview: `&line=N` 渲染模式滚动到最近标题段落，不强制切换源码视图
- Preview: HTML 源码/编辑模式 srcPre.style.display 同步修复（scrollToCodeLine/toggleRawMode/applyMarkdownModeView 3 处）
- Preview: HTML 编辑模式 textarea 跨父节点插入修复（htmlContentWrap 定位）
- Preview: Markdown 源码高亮 textContent+code 重复修复
- Search: clearSearch IIFE 猴子补丁移到事件绑定之前
- 内容匹配: cmd-file-header 插入 spacer 替代 flex:1 链接 + chevron 方向修正

## v1.46 Agent Panel 空白修复 + 双重面板修复 ✅ (commit: 2026-06-28)
- Fix: agent panel grid-column 错位导致空白（auto-placement→显式 grid-column:4）
- Fix: createTerminal() 容器尺寸检测加固（<50px 回退 600px）
- Fix: CSS transition 期间跳过 fit（doFit 守卫），ResizeObserver/win-resize 统一
- Fix: connectWs() 优先用预估算 _agentInitCols 而非可能缩水后的 term.cols
- Fix: OpenClaw 后端 agent panel 打开/关闭时内容跳动（.content + transition）
- Fix: Preview agent/feedback 面板同时弹出（CSS display:none + JS inline display 清理）
- Fix: agent.js doFit is not defined（openclaw 下不创建 terminal 时加 typeof 守卫）
- Fix: closeRightSidebar() 内聚 _desktopPollTimer + _stopSidebarRefresh 清理
- Feat: OpenClaw 输入框 placeholder 添加 /clawmate project 提示
- Docs: README 业务架构图（flowchart LR 替换技术实现图）

## v1.47 Preview &line=N + 搜索清除 + 内容匹配面板 + 源码高亮 ✅ (2026-06-29)
- Preview: `&line=N` 渲染模式滚动到最近标题段落，不强制切换源码视图
- Preview: HTML 源码/编辑模式 srcPre.style.display 同步修复
- Preview: HTML 编辑模式 textarea 跨父节点插入修复
- Preview: Markdown 源码高亮 textContent+code 重复修复
- Search: clearSearch IIFE 移到事件绑定之前
- 内容匹配: cmd-file-header spacer 替代 flex:1 + chevron 方向修正

## v1.48 分享功能增强 + 后端新 API ✅ (2026-07-01)
- 卡片 ⋯ 菜单"生成分享链接 / 取消分享"
- preview 分享按钮根据状态高亮
- GET /api/clawmate/share/active + POST /api/clawmate/share/expire
- 删除 thumb 上 card-share-icon 叠加图标

## v1.49 Session History 增强 ✅ (2026-07-05)
- 根级会话支持、输入批处理 1.5s idle 合并
- 跨 session 污染防护、Transcript 匹配优化
- "分析文件" prompt 同步写入 .chat.jsonl
- 计数修正、会话数据规范化、浏览器目录感知
- Session API 返回统计字段、CSS/UI 统一

## v1.50 会话历史增强 + 预览行号重构 + 添加到会话 ✅ (2026-07-07)
- cwd 恢复、Transcript 作用域限定、文件上下文保留
- 代码预览 CSS Grid → 逐行 Flex 重构
- "添加到会话" 桌面浮层、画廊/列表菜单集成
- Agent 焦点修复、测试新增

## v1.51 分页阈值 + 搜索优化 + 文件上下文去重 ✅ (2026-07-09)
- 分页阈值调整：grid 66 条、list 21 条、session history 20 条
- card-match-badge 样式对齐 card-menu-btn（22px 方形）
- 搜索结果文案统一、列表徽标颜色修复亮色主题
- 内容匹配弹窗居中（Agent 面板打开时自动偏移）
- 文件上下文去重：last_injected_file → known_files Set
- WS 重连文件上下文保留修复、Transcript 收集路径修复
- "路径"→"位置" 按钮重命名、导航到父目录
