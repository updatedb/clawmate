...
(function (global) {
  ...
  function mount(options) {
    ...
    function render() {
      ...
      var html = data.status_summary ? '<section class="project-panel-section project-panel-summary"><b>项目摘要</b><p class="project-panel-hint">' + esc(data.status_summary) + '</p></section>' : '';
      html += '<div class="project-status-bar"><span>更新于 ' + esc((summary.refreshed_at || '').replace('T', ' ').slice(0, 16)) + '</span><span data-project-status>执行中 ' + (summary.running || 0) + ' · 待处理 ' + (summary.pending || 0) + ' · 失败 ' + (summary.failed || 0) + '</span></div>';
  ...
  function mountPreview() {
    ...
    function open() {
      ...
      panel.innerHTML = '<header class="project-panel-header"><strong>项目面板</strong><button class="panel-close-btn" aria-label="关闭项目面板"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button></header><div class="project-panel-body">加载项目概况…</div>';
      ...
      controller = mount({body:panel.querySelector('.project-panel-body'), getContext:function () { return {root:root, project:project}; }, isOpen:function () { return Boolean(panel); }, openFeedback:openFeedback});
      controller.refresh();
    }
