...
**该绑定的实际后果比"文件访问"更宽，必须显式记录。** 因为 `local-admin` 同时被绑定为 `request.state.session` 与 `request.state.user`，设置类接口（`settings_routes._admin` 要求两者皆为真）对命中本机绕过的客户端**同样开放**：它能读取账户列表、创建账户、改授权、删用户。也就是说 `auth.local_hosts` 中的主机（含主机名，如 `openclaw.lan`）无需密码即拥有完整管理员能力，而不仅是文件访问。`/api/clawmate/auth/status` 保持 cookie 语义只挡住**浏览器界面**，挡不住直接调用接口。若这个信任边界过宽，收窄点在 `_is_local_client`：把管理员主体限定为回环地址，并让 `local_hosts` 只获得非管理员身份。

**因此客户端 IP 的解析本身是安全关键。** `x-forwarded-for` 只在直连对端本身就是受信本机时才被采纳，且取**最后一跳**——该头在其余情况下完全由调用方控制，会让任意远程客户端自称回环并继承本机信任；而常见 nginx 指令 `$proxy_add_x_forwarded_for` 会把客户端自填的值保留在开头、把代理真实看到的地址追加在末尾，所以首个元素是可伪造的那个。代理采用替换式写法时只有一个元素，首尾相同。
...
校验职责按层划分：`RootRegistry` 校验 root 自身（存在性、越界、`dir` 唯一、`id` 唯一、引用完整性）；`UserStore` 只校验授权项的**形状与字符集**以及「普通用户至少一个授权」；**授权 id 是否存在于注册表由设置路由校验**，不存在返回 422。这样两个 store 互不知晓，由路由组合——与 `RootRegistry.delete(referenced_by=...)` 让注册表不感知用户是同一原则。注意授权 id 是注册表引用，不是文件系统路径：`id="3gpp"` 对应 `dir="helper/3gpp"`，不能用 id 去拼路径做校验。
