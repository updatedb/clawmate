...
def test_edge_surfaces_consume_shared_tokens_and_recoverable_share_copy():
    login_css = (ROOT / "src" / "static" / "css" / "login.css").read_text(encoding="utf-8")
    onlyoffice_html = (ROOT / "src" / "static" / "onlyoffice.html").read_text(encoding="utf-8")
    share_html = (ROOT / "src" / "static" / "share-view.html").read_text(encoding="utf-8")
...
def test_mobile_preview_prioritizes_document_context_over_secondary_tools():
    ...
    style_css = (ROOT / "src" / "static" / "css" / "style.css").read_text(encoding="utf-8")
    preview_css = (ROOT / "src" / "static" / "css" / "preview.css").read_text(encoding="utf-8")
